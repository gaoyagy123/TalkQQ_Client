...
CPictureEx::CPictureEx()
{
	...
	m_hBackgroundBrush = NULL;//ZhangHuaiLiang add code 2012-06-06
	m_nGlobalCTSize    = 0;
	m_nCurrOffset	   = 0;
	m_nCurrFrame	   = 0;
	m_nDataSize		   = 0;
...
//			FillRect(m_hMemDC,&rect,m_hBackgroundBrush);
//			DeleteObject(hBrush);
//		};
		RECT rect = {
			m_arrFrames[m_nCurrFrame].m_frameOffset.cx,
...
HBRUSH CPictureEx::GetBkBrush() const
{
...
{
	SetWindowPos(NULL,0,0,nWidth,nHeight,SWP_NOMOVE | SWP_NOZORDER /*| SWP_HIDEWINDOW*/);

	//CWnd * pWndTopLevelParent = GetTopLevelParent();
	//if( pWndTopLevelParent )
	//{
	//	LONG dwStyle = GetWindowLong( pWndTopLevelParent->GetSafeHwnd(),GWL_STYLE );
...
void CPictureEx::OnTimer( UINT nIDEvent )
{
	...
	//	{
	//		LONG dwExStyle = GetWindowLong( pWnd->m_hWnd,GWL_EXSTYLE );
	//		if( ( dwExStyle & WS_EX_TOPMOST ) == 0 )
	//		{
	//			pWnd->SetWindowPos( &wndTopMost,0,0,0,0,SWP_NOMOVE | SWP_NOSIZE );
	//			pWnd->RedrawWindow();
	//		}
	//		if( SetBkTransparent(0) )
	//		{
	//			KillTimer( nIDEvent );
	//			ShowWindow( SW_SHOW );
	//		}
...
BOOL CPictureEx::SetBkTransparent( CDC * pDC )
{
	...
	if( pWnd )
	{
		...
		if( pDC )
		{
			...
			{
				CBitmap MemBmp;
				if( MemBmp.CreateCompatibleBitmap( pDC,rc.Width(),rc.Height() ) )
				{
					CBitmap * pOldBitmap = MemDC.SelectObject( &MemBmp );
					MemDC.BitBlt( 0,0,rc.Width(),rc.Height(),pDC,rc.left,rc.top,SRCCOPY );
					if( m_hBackgroundBrush ) DeleteObject( m_hBackgroundBrush );
					m_hBackgroundBrush = CreatePatternBrush( (HBITMAP)MemBmp.GetSafeHandle() );
